...
def build_from_orig(version="ss", new_d=None, run=True, r_d='..'):
    ...
    # Add DIS
    dis_o = m_org.dis
    # updated model top from Simon, end 2021
    #excel_file = os.path.join("..", "data", "model_top", 
    #                      "modelpoint_values_DEM2021.xls"
    #                     )
    #nmtp = pd.read_excel(excel_file, header=0, index_col=0).dropna(subset=['DEM_2021_V'])
    ...
    # setup spatial reference crap
    # if flopy gets fixes this might work:
    try:
        ...
        # this throw deprecation warnings now
        # -- but flopy is broken for the alternative *sigh*
        m.sr = flopy.utils.reference.SpatialReference.from_gridspec(
            os.path.join("..","prev_models","SthDn_Trans_Scen0",
                     "skinny_wo_glo","SthDn_Trans_mod1.spc")
        )
        #m.sr.proj4_str = "EPSG:2193"
        # m.sr.epsg = 2193
...
#     plt.figure()
#     plt.imshow(ib[0], interpolation='none')
#     plt.colorbar()
...
#     plt.colorbar()
    ...
    m.bas6.ibound = ib
#     plt.figure()
#     plt.imshow(m.bas6.ibound.array[0], interpolation='none')
#     plt.colorbar()
    # Add RCH
    rch_o = m_org.rch
    ...
    if version == 'ss':
        rech = 0.8 * rchmult * (0.673/365.25)  ### check this prior
    elif version == 'pred':
        rech = 0.8 * rchmult * (0.673/365.25)  ### check this prior
...
def setup_pst(new_m, nreals=1000, vis_cov=False, r_d='..'):
    ...
    # K pars
    # different variagram than drains
    ...
    # grid based vK
    #pf.add_parameters("vka1.ref", # array like file
    #                  par_type='grid', 
    #                  lower_bound=0.01, 
    #                  upper_bound=100.,
    #                  par_name_base='vk-gr',
    ...
    # global constant coastal ghb conductance mult? 
    ...
    if period == 'proj':
        # temporal recharge pars (TODO: poss also req for a transient history model)
        for f in rch_files:
    ...
    # dummy par for tracking spurious adjustments
    dp_df = pd.DataFrame([1],
                         columns=['noinfl'],
                         index=[1]).rename_axis(index='idx').to_csv(
                         os.path.join(pf.new_d, "dummypar.csv"))
    pf.add_parameters(filenames="dummypar.csv", par_type='constant',
                      index_cols='idx', use_cols='noinfl',
                      par_name_base=f'dumpar',
